...
/*
这个方法的作用是返回请求和最大值的较小值，因为最大值是配额组的资源上限。根据注释中所描述的多层次配额模型，在向上传递请求时，传递超过其最大值的请求将导致错误/无效的运行时分配。举例说明，如果父配额组的最大值为 20，子配额组的最大值为 10，而子配额组的请求为 30。如果子配额组将 30 的请求向上传递，并获得 20 的运行时配额（受限于父配额组的最大值为 20），则子配额组只能使用 10（受其最大值的限制）
*/
